...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['register'])) {
        ...
        $password = trim($_POST['password']); // Lưu mật khẩu dạng plain text

        // Tách họ và tên
        $name_parts = explode(' ', $full_name);
        $last_name = array_pop($name_parts); // Lấy tên cuối cùng (last_name)
    ...
    if (isset($_POST['login'])) {
        // Xử lý đăng nhập
        $email = trim($_POST['email']);
        $password = trim($_POST['password']); // So sánh trực tiếp mật khẩu
